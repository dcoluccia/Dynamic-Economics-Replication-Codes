...
class simulate:
    ...
    rho   : persistence of income process, optional (default = 0.95)
            array_like(float)
    mu    : drift of income process, optional (default = 0)
            array_like(float)
            
    Calibrated parameters
    R     : gross interest rate, optional (default = 1.1)
            array_like(float)
...
#        w = []
#
#        w0 = data.loc[data['id'] == (i+1)]['income'].values.tolist()[0]#W_grid[4]#np.random.choice(W_grid, size = 1).tolist()[0] ; 
#        w.append(w0)
#        
...
#            return array[idx]
#        i0 = W_grid.tolist().index(find_nearest(W_grid,w0))
#        
